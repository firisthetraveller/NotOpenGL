...
TEST_CASE("Boids are generated in the pond") {
  for (unsigned int i = 0; i < 100; i++) {
    Fish fish = Fish();
    CHECK((fish.getPosition().x > -1 && fish.getPosition().x < 1.0 && fish.getPosition().y > -1 && fish.getPosition().y < 1.0 && fish.getPosition().z > -1 && fish.getPosition().z < 1.0));
  }
}
...
TEST_CASE("Boids can detect each other") {
  ...
  std::shared_ptr<Fish> fish2 = std::make_shared<Fish>(
    glm::vec3{fish->getPosition().x + Config::get().VISUAL_RANGE / 2, fish->getPosition().y, fish->getPosition().z},
  ...
  std::shared_ptr<Fish> fish3 = std::make_shared<Fish>(
    glm::vec3{fish->getPosition().x + fish->getRadius() + 2 * Config::get().VISUAL_RANGE, fish->getPosition().y, fish->getPosition().z},
    fish->getRadius(), fish->getRotationX()
  );
