...
public:
    ...
    {
        return [&](std::shared_ptr<FishData>& fish, Environment&) {
            if (fish->_center.x < -Config::get().ASPECT_RATIO + Config::get().WALL_MARGIN)
            {
    ...
    static Behavior foodSeeking()
    {
        return [&](std::shared_ptr<FishData>& fish, Environment& env) {
            ...
            for (auto& food : env.foods)
            {
                ...
                float distance =
                    Geometry::distance2D(food->getPosition(), fish->_center) - food->getHitbox();
    ...
    static Behavior avoidObstacles()
    {
        return [&](std::shared_ptr<FishData>& fish, Environment& env) {
            ...
            {
                if (fish->isNear(other->_center, 0.0f, Config::get().OBSTACLE_DETECTION_RADIUS))
                {
                    movement += (fish->_center - other->_center) * (other->_radius / Geometry::distance2D(fish->_center, other->_center));
                }
            }
