...
static void eatingTime(std::vector<std::shared_ptr<Fish>>& fishs, std::vector<std::shared_ptr<Food>>& foods)
{
    for (auto& fish : fishs)
    {
        ...
        for (auto& food : foods)
        {
            if (food->exists() && Geometry::distance2D(food->getPosition(), fish->getData()->_center) < food->getHitbox())
            {
...
template<typename T>
std::vector<std::shared_ptr<T>> generate(int count)
{
...
int main(int argc, char* argv[])
{
    ...
    ctx.update = [&]() {
        ...
        Environment::getInstance().foods =
            filter(Environment::getInstance().foods, Predicate<std::shared_ptr<Food>>([](std::shared_ptr<Food>& f) { return f->exists(); }));
